...
func main() {
	...
	var level int8
	if clampedLevel := min(verbosityLevel, math.MaxInt8); clampedLevel >= 0 && clampedLevel <= math.MaxInt8 {
		level = int8(clampedLevel)
	}
	logf.SetLogger(zap.New(zap.Level(zapcore.Level(-1*level)), zap.UseDevMode(debug)))
