...
func GetAvailableSpace(path string) (int64, error) {
	...
	if stat.Bavail > math.MaxInt64 {
		return int64(-1), fmt.Errorf("available blocks count %d exceeds int64", stat.Bavail)
	}
	if stat.Bavail <= math.MaxInt64 {
		...
		if avail > math.MaxInt64/bsize {
			return int64(-1), fmt.Errorf("available space overflow: %d * %d exceeds int64", avail, bsize)
		}
